...
public class BaseRefreshLayout extends LinearLayout {
     ...
     * 网络异常显示界面
     */
    private View mNetworkAnomalyView;
    /**
    ...
    public BaseRefreshLayout(Context context, AttributeSet attrs) {
        ...
        initWholeHeaderView();
        setRefreshViewHolder(new NormalRefreshViewHolder(context, true));
        initNetworkView();
    }
    ...
    private void initNetworkView(){
        mNetworkView  = new LinearLayout(getContext());
        mNetworkView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        mNetworkView.setOrientation(LinearLayout.VERTICAL);
    ...
    public void setCustomHeaderView(View customHeaderView, boolean scrollable) {
        ...
        if (mCustomHeaderView != null) {
            mCustomHeaderView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
     ...
     * 设置网络异常界面
     */
    public void setNetworkAnomalyView(View networkAnomalyView, boolean isShow) {
        if (networkAnomalyView != null && networkAnomalyView.getParent() != null) {
            ViewGroup parent = (ViewGroup) networkAnomalyView.getParent();
            parent.removeView(networkAnomalyView);
        }
        mNetworkAnomalyView = networkAnomalyView;
        if (mNetworkAnomalyView != null) {
            mNetworkAnomalyView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
            mNetworkView.addView(mNetworkAnomalyView);
        }
        if(isShow){
            showNetworkAnomaly();
        }else{
    ...
    private boolean handleActionMove(MotionEvent event) {
        ...
        if (refreshDiffY > 0 && shouldHandleRefresh() && isCustomHeaderViewCompleteVisible()&&mNetworkView.getVisibility()==GONE) {
    ...
    public interface RefreshLayoutDelegate {
         ...
         * 开始刷新
         */
        void onRefreshLayoutBeginRefreshing(BaseRefreshLayout refreshLayout);
         ...
         */
        boolean onRefreshLayoutBeginLoadingMore(BaseRefreshLayout refreshLayout);
    }
